...
class AnnotationsParser():
    ...
    def _json_to_df(self):
        ...
        Output:
            ...
            cats, anns, img - dataframes for each of the three relevant kinds of information in 
                .json file
        '''
        ...
        img_df = pd.DataFrame.from_dict(img).set_index('id').drop(to_del, axis=1)
        img_df.rename_axis('image', inplace=True)

        output = anns_df.join(img_df, on='image').drop(['area', 'segmentation', 'bbox'], axis=1)        
